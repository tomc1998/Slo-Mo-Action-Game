...
Rect Rect::inset(EdgeInsets &insets) const {
  return Rect(pos.x + insets.left, pos.y + insets.top,
...
void Rect::clip_line_segment(Vec2 &p0, Vec2 &p1) const {
}

Rect Rect::translate(f32 x, f32 y) const {
  return Rect(pos.x + x, pos.y + y, size.x, size.y);
}

Rect Rect::translate(Vec2 v) const { return Rect(pos + v, size); }

Rect Rect::unconstrained() const { return Rect(pos, Vec2(-1.f, -1.f)); }

f32 Rect::centre_x() const { return pos.x + size.y / 2.f; }

f32 Rect::centre_y() const { return pos.y + size.y / 2.f; }

Vec2 Rect::centre() const { return Vec2(centre_x(), centre_y()); }

f32 Rect::top() const { return pos.y; }

f32 Rect::bottom() const { return pos.y + size.y; }

f32 Rect::left() const { return pos.x; }

f32 Rect::right() const { return pos.x + size.x; }
