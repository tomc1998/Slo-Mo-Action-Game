    ...
    for (u32 ii = 0; ii < ecs->comp_player_controlled.size(); ii++) {
      for (u32 jj = 0; jj < ecs->comp_game_entity.size(); jj++) {
        ...
        // Render attacking 'arc'
          ...
          const static u32 NUM_SEGMENTS = 6;
          const static f32 THICKNESS = p.ATTACK_ARC_MAX_DIS - p.ATTACK_ARC_MIN_DIS;
          const static f32 START_RAD = p.ATTACK_ARC_MIN_DIS;
          ...
          f32 seg_len = p.ATTACK_ARC_LEN / (f32)NUM_SEGMENTS;
