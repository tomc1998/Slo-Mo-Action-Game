...
Shader::Shader(const char *vert_shader_path, const char *frag_shader_path,
  ...
  glShaderSource(vert_shader, vert_shader_src.size(), &(vert_shader_src[0]),
                 &(vert_shader_src_line_lengths[0]));
  glShaderSource(frag_shader, frag_shader_src.size(), &(frag_shader_src[0]),
                 &(frag_shader_src_line_lengths[0]));
  ...
  for (auto loc_query : uniform_loc_queries) {
    *loc_query.second = glGetUniformLocation(program_id, loc_query.first);
    if (*loc_query.second == -1) {
      throw std::runtime_error(
          std::string("Uniform with name '") + loc_query.first +
          "' not found in shader " + "(vertex shader: '" + vert_shader_path +
